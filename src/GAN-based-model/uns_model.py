...
class model(object):
    ...
    def train(self, config, sess, saver, data_loader, dev_data_loader=None, aug=False):
        print ('TRAINING(unsupervised)...')
        if aug: get_target_batch = data_loader.get_aug_target_batch
        else: get_target_batch = data_loader.get_target_batch
        ...
        for step in range(1, config.step+1):
            ...
            for _ in range(config.gen_iter):
                ...
                gen_loss, seg_loss, _, smaple = sess.run(run_list, feed_dict=feed_dict)
            ...
            if step % config.print_step == 0:
                print (f'Step: {step:5d} dis_loss: {step_gen_loss:.4f} gen_loss: {step_dis_loss:.4f} seg_loss: {step_seg_loss:.4f}')
                step_gen_loss, step_dis_loss, step_seg_loss = 0.0, 0.0, 0.0
